...
def get_dags():
    qry = f"SELECT * FROM D_EWAYA_CONFIG.GD_AFDag WHERE state=1"
    return getWebQuery(qry).json()['result']

def get_tasks_by_dag_id(dag_id):
    qry = f"SELECT * FROM D_EWAYA_CONFIG.GD_AFTask WHERE state=1 and dag_id={dag_id}"
    return getWebQuery(qry).json()['result']

def get_dag_by_id(dag_id):
    qry = f"SELECT * FROM D_EWAYA_CONFIG.GD_AFDag WHERE state=1 and dag_id={dag_id}"
    return getWebQuery(qry).json()['result']
